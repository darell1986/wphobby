jQuery(document).ready(function($) {
	...
	var buttonPosition = ( $( headingPosition + ' a' ).length ) ? headingPosition + ' a' : headingPosition + '+a' ;
	var buttonSelector = $( buttonPosition ).attr('class'); // get "Add New Widget" button selector class
	...
	var adminLinks = '<a href="' + ContentAd.newWidgetCall + '" class="' + buttonSelector + ' thickbox">Add New Widget</a>'
		...
		+ '<a href="' + ContentAd.instructionsnCall + '" class="' + buttonSelector + ' thickbox">' + ContentAd.instructionsLinkText + '</a>';

	$( headingPosition ).attr('style','display:inline-block;margin-bottom:0.2em;');
	$( headingPosition ).prepend( '<img id="logo" src="' + ContentAd.pluginsUrl + '/contentad/images/ca_logo.png" alt="Content.ad" />' );
	...
	function lastEdited() {
		var last_edited = $('.last_edited abbr');
		for(var i = 0; i < last_edited.length; i++) {
			var le = new Date( $(last_edited[i]).attr('title') );
			var le_am_pm = (le.getHours() > 11 && le.getHours() < 24) ? 'pm' : 'am' ;
			var le_hours = (le.getHours() > 12) ? le.getHours() -12 : le.getHours() ;
			var le_text = le.getFullYear() + '/' + addZero((le.getMonth()+1)) + '/' + addZero(le.getDate());
