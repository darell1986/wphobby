...
if( !class_exists( 'wip_custom_login_panel' ) ) {
	...
	class wip_custom_login_panel {
		...
		public function admin_menu() {
			...
			add_menu_page(
				__('WIP Custom Login', 'wip-custom-login'),
				__('WIP Custom Login', 'wip-custom-login'),
				'administrator',
		...
		public function add_script() {
			 ...
			 if ( $pagenow == 'admin.php' ) {
				...
				wp_enqueue_style ( 'WIP_plugin_panel', $file_dir.'css/panel.css' ); 
				wp_enqueue_style ( 'WIP_plugin_panel_on_off', $file_dir.'css/on_off.css' );
				wp_enqueue_style ( 'WIP_plugin_panel_googlefonts', '//fonts.googleapis.com/css?family=Roboto');
				
				wp_enqueue_media();
				wp_enqueue_script( 'jquery');
				wp_enqueue_script( "jquery-ui-core", array('jquery'));
				wp_enqueue_script( "jquery-ui-tabs", array('jquery'));
				wp_enqueue_script( 'WIP_plugin_panel_on_off', $file_dir.'js/on_off.js','3.5', '', TRUE); 
				wp_enqueue_script( 'WIP_plugin_panel', $file_dir.'js/panel.js',array('jquery','media-upload','thickbox','wp-color-picker'),'1.0',TRUE ); 
					...
					if ( isset($element['tab']) && $element['tab'] == $_GET['tab'] ) {
						...
						foreach ($element as $value ) {
							...
							if ( isset($_REQUEST['element-opened']) && $_REQUEST['element-opened'] == "Skins" ) {
								...
								$message_action = __('Options saved successfully.');
							...
							$message_action = __('Options saved successfully.', 'wip-custom-login' );
						...
						case 'navigation': ?>
							...
							<div id="WIP_plugin_panel_tabs">
								...
								<div id="WIP_plugin_panel_header">
									...
									<div class="plugin_description"> <h2 class="maintitle"> <?php echo __( 'WIP Custom Login','wip-custom-login'); ?> </h2> </div>
							...
							if (isset($value['type'])) :
								...
								case "color": ?>
									...
									<div class="WIP_plugin_panel_box">
										...
										<label for="<?php echo $value['id']; ?>"><?php echo $value['name']; ?></label>
										<input type="text" name="<?php echo $value['id']; ?>" id="<?php echo $value['id']; ?>" value="<?php if ( wip_custom_login_setting($value['id']) != "") { echo wip_custom_login_setting($value['id']) ; } else { echo $value['std']; } ?>" data-default-color="<?php echo $value['std']; ?>" class="WIP_plugin_panel_color"  />
										<p><?php echo $value['desc']; ?></p>
